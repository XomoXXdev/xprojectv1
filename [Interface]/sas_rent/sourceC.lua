	...
	if rentShow and not getPedOccupiedVehicle(localPlayer) then
		...
		for k,v in ipairs(vehList) do
			if vehCount < 10 then
				...
				dxDrawText("Bérlés ára: "..tostring(v[2]).. " $",panel[1]+210,panel[2]+15-50 + (vehCount * 120),panel[1]+410,0,tocolor(0,0,0,255),1,font1,"right","top",true,true,true,true)
				dxDrawText("Bérlés ára: "..tostring(v[2]).. " $",panel[1]+211,panel[2]+15-50 + (vehCount * 120),panel[1]+411,0,tocolor(255,255,255,255),1,font1,"right","top",true,true,true,true)
