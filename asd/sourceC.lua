...
local changingProcess = 25 -- // Sets how many times you have to cultivate MAX : 100
local changeValue = 0
...
local alpha = 0
--local worldX, worldY = -25.6328125, -364.9140625
...
local montSerrat = dxCreateFont("files/fonts/montSerrat.otf", 10)
local montSerrat12 = dxCreateFont("files/fonts/montSerrat.otf", 12)
local montSerrat9 = dxCreateFont("files/fonts/montSerrat.otf", 9)
local montSerratBold10 = dxCreateFont("files/fonts/montSerratBold.ttf", 10)
local montSerratBold11 = dxCreateFont("files/fonts/montSerratBold.ttf", 11)
local montSerratBold = dxCreateFont("files/fonts/montSerratBold.ttf", 12)
local montSerratBold17 = dxCreateFont("files/fonts/montSerratBold.ttf", 17)
...
--	{"files/models/lettuce", "files/models/lettuce", 16676},
	...
	for i = 1, 50 do
		...
		if column == 3 then
			table.insert(defaultBlockTable, {
				...
				healthTick = 0,
				-- # Identify block by row and column
				blockRow = row,
				blockColumn = column,
			})
		else
			table.insert(defaultBlockTable, {
				state = "land", -- TODO: back "land"
				newState = false,
				newStateLevel = 0,
				newStateSaveLevel = 0,
				changingTick = 0,
...
addEventHandler("onClientRender", getRootElement(), function()
	...
	-- # Draw the actual farm ground to a render target
	dxSetRenderTarget(renderTarget, true)
		for k, v in pairs(blockTable[renderData.farmDimension]) do
			if v.blockColumn ~= 3 then
				if v.waterLevel < 100 then
					dxDrawImage((v.blockColumn-1)*128,v.blockRow*128,128, 128,"files/"..v.state..".png") -- # Actual lands
					if v.newState then
						dxDrawImage((v.blockColumn-1)*128,v.blockRow*128,128, 128,"files/"..v.newState..".png", 0,0,0, tocolor(255,255,255,255 * (v.newStateLevel/100))) -- # New state to the surface
					end
				end
			else
		...
		for k, v in pairs(blockTable[renderData.farmDimension]) do
			if v.blockColumn ~= 3 then
				...
				if v.state then
					if v.waterLevel > 0 then
						dxDrawImage((v.blockColumn-1)*128-64,v.blockRow*128-64,256, 256,"files/"..v.state.."_wet.png",0,0,0,tocolor(255,255,255,255 * (v.waterLevel/100))) -- # Actual lands
						dxDrawImage((v.blockColumn-1)*128-64,v.blockRow*128-64,256, 256,"files/wet.png",0,0,0,tocolor(255,255,255,255 * (v.waterLevel/400))) -- # Actual lands
						if v.newState then
							dxDrawImage((v.blockColumn-1)*128-64,v.blockRow*128-64,256, 256,"files/"..v.newState.."_wet.png", 0,0,0, tocolor(255,255,255,255 * (v.newStateLevel/100))) -- # New state to the surface
						end
					end
				end
			end
		...
		for k, v in pairs(blockTable[renderData.farmDimension]) do
			if v.blockColumn == 3 then
				-- # Draw a path in the middle of the farm
	...
	if getElementData(localPlayer, characterIDElementData) == getElementData(farmMarker, "farm:owner") then -- # Show the management table only if the player is owner
		...
		if manageX and manageY then
			...
			if distance < 3 then
				...
				if not renderData.permissionState then
					...
					if renderData.actualEditing == "farm:board" then
						boardText = renderData.invitingText
						changeNameButtonText = getTranslatedText("changeFarmName_button")
						-- # Cursor indicator
						if cursorState then
							local w = dxGetTextWidth(renderData.invitingText, distMul * 0.8, montSerratBold17)
							if utf8.len(boardText) >= 11 then
								cursorTickCorrectY = 23
								w = dxGetTextWidth(utf8.sub(renderData.invitingText, 12, utf8.len(boardText)), distMul * 0.8, montSerratBold17)
							end
					...
					dxDrawRectangle(x, y + (24) * distMul, 150, 150, tocolor(0,0,0,130))
					dxDrawRectangle(x, y + (24) * distMul, 150, 25, tocolor(0,0,0,130))
					dxDrawText(getTranslatedText("farmManagementTitle"), x, y + (24) * distMul, x + sx+10, y + (24) * distMul+25, tocolor(255, 255, 255, 255 * alphaMul), distMul * 0.9, montSerratBold, "center", "center")
					dxDrawText(boardText, x, y + (24) * distMul+35, x + sx+10, y + (24) * distMul+55, tocolor(255, 255, 255, 255 * alphaMul), distMul * 0.8, montSerratBold17, "center", "top")
					dxDrawButtonWithText(x+5, y + (24) * distMul + 125, 140, 20, tocolor(61, 108, 165, 200), tocolor(61, 108, 165, 255), getTranslatedText("permission_button"), tocolor(0,0,0,255), 1, montSerrat, "center", "center", "permissionButton")
					dxDrawButtonWithText(x+5, y + (24) * distMul + 100, 140, 20, tocolor(61, 108, 165, 200), tocolor(61, 108, 165, 255), changeNameButtonText, tocolor(0,0,0,255), 1, montSerrat, "center", "center", "editFarmName")
				else
					...
					permissionSelected = ""
					permissionKey = 0
					dxDrawRectangle(x, y + (24) * distMul, 300, 25, tocolor(0,0,0,200))
					dxDrawText(getTranslatedText("permission_button"), x, y + (24) * distMul, x + sx+10, y + (24) * distMul+25, tocolor(255, 255, 255, 255 * alphaMul), distMul * 0.9, montSerratBold, "center", "center")
					if renderData.actualEditing == "" then
						dxDrawRectangle(x, y + (24) * distMul+25, 300, 25+plusPermissionWidth, tocolor(0,0,0,130))
						dxDrawButtonWithText(x+3, y + (24) * distMul + 27+plusPermissionWidth, 130, 20, tocolor(61, 108, 165, 200), tocolor(61, 108, 165, 255), getTranslatedText("permission_addNewMember"), tocolor(0,0,0,255), 1, montSerrat, "center", "center", "newMemberButton")
						dxDrawButtonWithText(x+237, y + (24) * distMul + 27+plusPermissionWidth, 60, 20, tocolor(61, 108, 165, 200), tocolor(61, 108, 165, 255), getTranslatedText("permission_save"), tocolor(0,0,0,255), 1, montSerrat, "center", "center", "closePermission")
						for k, v in pairs(permissionTable) do
							dxDrawText(v.playerName, x+5, y + (24) * distMul + 23 + (k*25)-25, x + sx+10, y + (24) * distMul+50 + (k*25)-25, tocolor(255, 255, 255, 255 * alphaMul), distMul * 0.9, montSerratBold, "left", "center")
							local counter = 0
						...
						if cursorState then
							local w = dxGetTextWidth(renderData.invitingText, distMul * 0.9, montSerratBold)
							dxDrawLine(x+8+w, y + (24) * distMul+28, x+8+w, y + (24) * distMul+45, tocolor(255, 255, 255, 255), 1, true)
						end
						dxDrawText(renderData.memberNameEdit, x+5, y + (24) * distMul + 23, x + sx+10, y + (24) * distMul+50, tocolor(255, 255, 255, 255 * alphaMul), distMul * 0.9, montSerratBold, "left", "center")
						dxDrawButtonWithText(x+237, y + (24) * distMul + 27, 60, 20, tocolor(61, 108, 165, 200), tocolor(61, 108, 165, 255), getTranslatedText("permission_add"), tocolor(0,0,0,255), 1, montSerrat, "center", "center", "addMemberDone")
					end
				end
			end
		end
	end
	-- # Draw the amount of hoes and spades and watercans in the farm interior
	local localX, localY, localZ = getElementPosition(localPlayer)
			...
			if isElement(object) and isElementStreamable(object) then
				if getElementData(object, "farm:spadeObject") or getElementData(object, "farm:hoeObject") or getElementData(object, "farm:waterCanObject") then
					...
					if isLineOfSightClear(objX, objY, objZ, localX, localY, localZ, false, false, false, false, false, false, false, localPlayer) then
						local screenX, screenY = getScreenFromWorldPosition(objX, objY, objZ)
						if screenX and screenY then
							...
							if distance < 3 then
								...
								dxDrawRectangle(x, y + (24) * distMul, 100, 40, tocolor(0,0,0,130))
								if getElementData(object, "farm:hoeObject") then
								...
								dxDrawText(text, x, y + (24) * distMul, x + sx, 0, tocolor(255, 255, 255, 255 * alphaMul), distMul * 1, montSerratBold, "center", "top")
								dxDrawText(numberOfObject.." "..getTranslatedText("piecesShortly"), x, y + (45) * distMul, x + sx, 0, tocolor(255, 255, 255, 255 * alphaMul), distMul * 1, montSerrat, "center", "top")
							end
						end
					end
				end
	...
	if isCursorShowing() then
		...
		for i = 1, 50 do
			...
			if column ~= 3 then -- // Avoid the road blocks
				if isCursorInterSect3D(hitX, hitY, worldX+(column-1), worldY+row, 1, 1) then
					...
					if not renderData.plantState then
						...
						dxDrawText(getTranslatedText("ground_wateringLevel"), surfaceX-infoW/2, surfaceY-infoH/2+5, surfaceX+infoW/2, 0, white, 1, montSerrat, "center", "top")
						...
						dxDrawText(getTranslatedText("ground_state").." "..statusText, surfaceX-infoW/2, surfaceY-infoH/2+47, surfaceX+infoW/2, 0, white, 1, montSerrat, "center", "top")
						if blockTable[renderData.farmDimension][selectedPlant].newState == "land_cultivated" then
							infoH = 90
							dxDrawRectangle(surfaceX-infoW/2+10, surfaceY-infoH/2+70, (infoW-20)*blockTable[renderData.farmDimension][i].newStateLevel/100, 13, tocolor(155,118,83,255))
						...
						elseif blockTable[renderData.farmDimension][selectedPlant].state == "land_planted" then
							...
							dxDrawRectangle(surfaceX-infoW/2+10, surfaceY-infoH/2+70, (infoW-20), 13, tocolor(255,255,255,30)) -- Health background
							dxDrawRectangle(surfaceX-infoW/2+10, surfaceY-infoH/2+70, healthW, 13, tocolor(124,197,118,255)) -- Health state
							-- Name
							dxDrawRectangle(surfaceX-infoW/2, surfaceY-infoH/2-25, infoW, 25, tocolor(0,0,0,255))
							dxDrawText(seedTable[blockTable[renderData.farmDimension][selectedPlant].plantIndex].name, surfaceX-infoW/2, surfaceY-infoH/2-25, surfaceX-infoW/2+infoW, surfaceY-infoH/2, white, 1, montSerrat, "center", "center")
							-- Growing progress
							dxDrawRectangle(surfaceX-infoW/2+10, surfaceY-infoH/2+110, (infoW-20), 13, tocolor(255,255,255,30))
							dxDrawRectangle(surfaceX-infoW/2+10, surfaceY-infoH/2+110, (infoW-20)*blockTable[renderData.farmDimension][i].sizeChanger, 13, tocolor(155,118,83,255))
							dxDrawText(getTranslatedText("ground_growing"), surfaceX-infoW/2, surfaceY-infoH/2+87, surfaceX+infoW/2, 0, white, 1, montSerrat, "center", "top")
							-- Harvest button
							dxDrawButtonWithText(surfaceX-infoW/2+5, surfaceY-infoH/2+148, (infoW-10), 20, tocolor(61, 108, 165, 200), tocolor(61, 108, 165, 255), getTranslatedText("harvest_button"), tocolor(0,0,0,255), 1, montSerrat, "center", "center")
						...
						elseif blockTable[renderData.farmDimension][selectedPlant].state == "land_hole" then
							...
							dxDrawButtonWithText(surfaceX-infoW/2+5, surfaceY-infoH/2+70, (infoW-10), 20, tocolor(61, 108, 165, 200), tocolor(61, 108, 165, 255), getTranslatedText("ground_fillTheHole"), tocolor(0,0,0,255), 1, montSerrat, "center", "center")
							dxDrawButtonWithText(surfaceX-infoW/2+5, surfaceY-infoH/2+95, (infoW-10), 20, tocolor(61, 108, 165, 200), tocolor(61, 108, 165, 255), getTranslatedText("ground_plantTheSeed"), tocolor(0,0,0,255), 1, montSerrat, "center", "center")
						else
							statusText = getTranslatedText("ground_uncultivated")
							infoH = 70
						end
					else
						local width, height = 210, 140
						local counter = 0
						dxDrawRectangle(surfaceX-width/2, surfaceY-height/2, width, height, tocolor(0,0,0,255))
						dxDrawText(getTranslatedText("ground_seedMenu"), surfaceX-width/2+10, surfaceY-height/2+7, surfaceX+width/2, 0, white, 1, montSerratBold, "left", "top")
						...
						for k, v in pairs(seedTable) do
							if (k > renderData.scrollNum and counter < renderData.maxSeedShow) then
								counter = counter + 1
								if isCursorInBox(surfaceX-width/2+10, surfaceY-height/2+35+(counter-1)*25, width-20, 20) then
									selectedSeed = k
								end
			...
			-- # Calculate the end of the column and reset
			if column == 5 then
				column = 0
				row = row + 1
			end
  ...
  if button == "left" and state == "down" then
		if clickedElement then
			...
			elseif getElementData(clickedElement, "farm:waterCanObject") then
				if hasPlayerPermission("wateringState") then
					-- TODO: add plus permission
					-- # Take the spade and hoe if exists
  	...
  	if selectedPlant > 0 then
					...
					if hasPlayerPermission("harvestState") then
						if not getElementData(localPlayer, "farm:wateringCan") and not getElementData(localPlayer, "farm:hoe") and not getElementData(localPlayer, "farm:spade") then
							if isElement(blockTable[renderData.farmDimension][selectedPlant].plantObject) and blockTable[renderData.farmDimension][selectedPlant].sizeChanger == blockTable[renderData.farmDimension][selectedPlant].plantFullSize then
								...
								--	givePlayerHarvestedPlant(seedTable[blockTable[renderData.farmDimension][selectedPlant].plantIndex], blockTable[renderData.farmDimension][selectedPlant].plantHealth)
			...
			if not blockTable[renderData.farmDimension][selectedPlant].changingState then
				-- # Cultivating by click
				if getElementData(localPlayer, "farm:hoe") then
					if calculatedRotation then
						if not blockTable[renderData.farmDimension][selectedPlant].newState or blockTable[renderData.farmDimension][selectedPlant].newState == "land" or blockTable[renderData.farmDimension][selectedPlant].newState == "land_cultivated" then -- // Cultivating is only allowed when the surface is default
...
addEventHandler("syncBlockTableClient", getRootElement(), function(player, table, delete, permissions)
	if not delete then
		...
		-- # Sync the permissions as well
		permissionTable = permissions
	else
		-- # Destroy the datas if the player leaves the farm
		for k, v in pairs(blockTable[renderData.farmDimension]) do
...
addEventHandler("syncNewSurfaceToClient", getRootElement(), function(selectedIndex, type, player, plantIndex)
	...
	elseif type == "cultivate" then
		...
		blockTable[renderData.farmDimension][selectedIndex].changingValue = blockTable[renderData.farmDimension][selectedIndex].newStateLevel+changingProcess
		blockTable[renderData.farmDimension][selectedIndex].changingState = true
		-- # Apply the visual things
	...
	elseif type == "watering" then
		...
		exports.bone_attach:setElementBoneRotationOffset(createdFarmModels[player], 0, -90, 0)
		exports.bone_attach:setElementBonePositionOffset(createdFarmModels[player], 0.2, 0.03, 0.2)
		setPedAnimation(player, "SWORD", "sword_IDLE", -1, true, false, false)

		-- # To get the object's right position we delay the effect creation
		setTimer(function()
		...
		setTimer(function()
			...
			setPedAnimation(player)
			exports.bone_attach:setElementBonePositionOffset(createdFarmModels[player], -0.05, 0.03, 0.3)
			exports.bone_attach:setElementBoneRotationOffset(createdFarmModels[player], 0, -180, 0)
...
addEventHandler("createFarmItemClient", getRootElement(), function(player, type, newValue)
	...
	if newValue then
		if type == "waterCan" then
			...
			exports.bone_attach:attachElementToBone(createdFarmModels[player], player, 12, -0.05, 0.03, 0.3, 0, -180, 0)
		elseif type == "hoe" then
			createdFarmModels[player] = createObject(16672, 0, 0, 0)
			setElementDimension(createdFarmModels[player], renderData.farmDimension)
			exports.bone_attach:attachElementToBone(createdFarmModels[player], player, 12, 0.2, 0.03, 0.05, 0, 90, -90)
		elseif type == "spade" then
			createdFarmModels[player] = createObject(16102, 0, 0, 0)
			setElementDimension(createdFarmModels[player], renderData.farmDimension)
			exports.bone_attach:attachElementToBone(createdFarmModels[player], player, 12, -0.28, 0.03, 0.05, 0, 0, 90)
		end
	end
end)
...
function changeBoardTexture(dbID, newName, boardElement)
	dxSetRenderTarget(boardTarget[dbID], true)
		...
		dxDrawText(newName, 0,0,256,128, tocolor(0, 0, 0, 255), 1, montSerratBold17, "center", "center")
	dxSetRenderTarget()
end
		...
		if boardName then
			...
			dxSetRenderTarget(boardTarget[boardID], true)
				...
				if boardOwner == 0 then
					dxDrawText(getTranslatedText("board_forRentText"), 0,0,256,128, tocolor(0, 0, 0, 255), 1, montSerratBold17, "center", "center")
				else
					dxDrawText(boardName, 0,0,256,128, tocolor(0, 0, 0, 255), 1, montSerratBold17, "center", "center")
				end
			dxSetRenderTarget()
			dxSetShaderValue(boardShader[boardID], "gTexture", boardTarget[boardID])
		end
...
addEventHandler("onClientRender", getRootElement(), function()
	...
	if #notificationData > 0 then
		for k, v in pairs(notificationData) do
			...
			dxDrawText(v.Name:gsub("\n", " "), renderData.interiorX+70, tilePosition-17, 60, tilePosition+renderData.interiorH, white, 1, montSerratBold, "left", "center")
			dxDrawText(getTranslatedText("board_enterInterior"), renderData.interiorX+70, tilePosition+20, 60, tilePosition+renderData.interiorH, white, 1, montSerrat, "left", "center")
		end
	end
end)
...
function hasPlayerPermission(permissionIndex)
	...
	for k, v in pairs(permissionTable) do
		...
		if tostring(getElementData(localPlayer, playerNameElementData):gsub("_", " ")) == tostring(v.playerName) then
			-- # Check if he has the specified permission
...
function dxDrawTick(x,y,state,toolTipText)
	-- # Draw the state of the permission
	if state then
	...
	if isCursorShowing() then
		...
		local textWidth = dxGetTextWidth(toolTipText, 1, montSerrat)
		if isCursorInBox(x,y,25,25) then
			dxDrawRectangle(relX+5, relY+5, textWidth+10, 25, tocolor(0,0,0,255), true)
			dxDrawText(toolTipText,relX+10, relY+5, 0,relY+30,white,1,montSerrat, "left", "center",false,false,true)
		end
	end
end
