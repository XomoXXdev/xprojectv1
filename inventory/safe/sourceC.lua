...
func.safe.nearby = function()
	if getElementData(localPlayer,"player:admin") >= 3 then
		...
		if cache.safe.show then
			...
			outputChatBox("#8163bf[xProject]:#ffffff Széf információ megjelenítve.",0,0,0,true)
		else
			removeEventHandler("onClientRender",getRootElement(),func.safe.render)
			outputChatBox("#8163bf[xProject]:#ffffff Széf információ eltüntetve.",0,0,0,true)
		end
	end
end
