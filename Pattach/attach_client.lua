function sendReadyMessage()
	triggerServerEvent("boneAttach_requestAttachmentData", resourceRoot)
...
function putAttachedElementsOnBones()
	for element,ped in pairs(attached_ped) do
		if ped then
			if isElement(ped) and isElement(element) and getElementType(ped) == "player" then
				if isElementOnScreen(ped) then
					...
					if isnan(objx) or isnan(objy) or isnan(objz) or isnan(offrx) or isnan(offry) or isnan(offrz) then
...
addEventHandler("onClientResourceStop", resourceRoot, function()
	...
	setElementData(root, "pb_attach_data", data, false)
end)
